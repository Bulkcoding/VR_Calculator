...
- placeholder가 마스킹 자격증명처럼 보이던 문제는 직전 커밋에서 해결됨

## 2026-06-22 — feat: 토스증권 추가 + 연동 모달 라벨/초기화 개선
- 토스증권 연동 대상 추가 (모달 드롭다운 / 연동 가능 목록 / API LABELS)
- 모달 라벨 변경: AppKey → ApiKey, AppSecret → ApiSecret
- 증권사 전환 시 ApiKey/ApiSecret/계좌번호 즉시 공란 초기화 (미연동 증권사)
- fetch stale 응답 가드: 전환 전 이전 증권사 응답이 폼을 잘못 채우지 않도록 무시
